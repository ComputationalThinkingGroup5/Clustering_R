...
The next command defines "shrinking population" as countries which have a negative value of the "popgrowth.rate" variable. 
```{r}
...
The following condition defines condition2 as.... 
...
Rename subset indexes and verify what input is:
```{r}
row.names(dfClus)=fromPy$country
head(dfClus)
```

Set random seed: this number must correspond wih any group members' seeds. 
```{r}
set.seed(999)
```
...
Apply function: need to indicate the amount of clusters required
```{r}
NumCluster=4
...
Request negative silhouettes: these are the ones which are poorly clustered. 
```{r}
...
Apply function: you need to indicate amount of clusters required. Install factoextra. 
```{r}
library(factoextra)
...
Adding results to original data frame: 
```{r}
